...
"""Federated client state and reusable runtime worker implementations."""
...
class FLClient:
    """Persistent per-client state that survives across rounds."""
    ...
    def move_state_to_device(self) -> None:
...
class FLClientWorker:
    """Reusable runtime worker bound to one executor thread."""
    ...
    def __init__(
        ...
        scaler_enabled = bool(self.use_amp and self.device.type == "cuda" and self.amp_dtype == torch.float16)
        ...
        if self.channels_last and self._runtime_on_device:
    ...
    def _move_runtime_model_to_device(self) -> None:
    ...
    def _offload_runtime_model_to_cpu(self) -> None:
    ...
    def _should_offload_runtime_model_after_round(self) -> bool:
    ...
    def train_and_upload(
        ...
        self._move_runtime_model_to_device()
        client.move_state_to_device()
        try:
            ...
            delta_w = self.local_trainer.train(
                ...
                old_local_weights=client.old_local_weights,
                generator=generator,
            ...
            if str(self.cfg.compressor.type) == "identity":
                ...
                compression_result = client.compressor.compress(
                    delta_w=delta_w,
                    model=self.model,
                    dataloader=client.dataloader,
                    device=self.device,
                ...
                if force_refresh_local_importance:
                    client.last_importance_refresh_round = int(round_num)
            ...
            if self.local_trainer.should_store_old_weights():
                client.old_local_weights = {
                    name: tensor.detach().clone()
                    for name, tensor in self.model.state_dict().items()
                }
            else:
                client.old_local_weights = None
            ...
            self.global_weights = None
            client.offload_state_to_cpu()
            if self._should_offload_runtime_model_after_round():
                self._offload_runtime_model_to_cpu()
